...
try {
    # Fix Slide Title font (Shape 1) on all content slides (Slides 2 to 8) to universal Segoe UI
    for ($i = 2; $i -le $pres.Slides.Count; $i++) {
        $s = $pres.Slides.Item($i)
        if ($s.Shapes.Count -ge 1) {
            $titleShp = $s.Shapes.Item(1)
            if ($titleShp.HasTextFrame -and $titleShp.TextFrame.HasText) {
                $tr = $titleShp.TextFrame.TextRange
    ...
    # Fix INIT'26 title font on cover slide
    for ($idx = 1; $idx -le $slide1.Shapes.Count; $idx++) {
        ...
        if ($shp.HasTextFrame -and $shp.TextFrame.HasText) {
            ...
            if ($txt -like "*INIT*") {
                ...
                $shp.TextFrame.TextRange.Font.Size = 90
            }
        }
    }
    ...
    Append-FormattedText $tr1 "TEAM: PHOENIX   |   TRACK: CYBERSECURITY   |   PROJECT: API SENTINEL" "Segoe UI" 16 $COLOR_WHITE $true | Out-Null
    ...
    Append-FormattedText $tr2 "TRACK : `r" "Segoe UI" 20 $COLOR_GREEN $true | Out-Null
    Append-FormattedText $tr2 "Cybersecurity`r`r" "Segoe UI" 18 $COLOR_WHITE $true | Out-Null

    Append-FormattedText $tr2 "PROBLEM STATEMENT : `r" "Segoe UI" 20 $COLOR_GREEN $true | Out-Null
    ...
    Append-FormattedText $tr2 $probText "Segoe UI" 15.5 $COLOR_TEXT_BODY $false | Out-Null

    Append-FormattedText $tr2 "TEAM NAME : `r" "Segoe UI" 20 $COLOR_GREEN $true | Out-Null
    Append-FormattedText $tr2 "Phoenix`r`r" "Segoe UI" 18 $COLOR_WHITE $true | Out-Null

    Append-FormattedText $tr2 "IDEA TITLE : `r" "Segoe UI" 20 $COLOR_GREEN $true | Out-Null
    $ideaText = "API Sentinel - Autonomous Real-Time API Security, Threat Detection & Zero-Trust Policy Enforcement Platform"
    Append-FormattedText $tr2 $ideaText "Segoe UI" 19 $COLOR_CYAN $true | Out-Null
    ...
    $tr = $tf.TextRange
    Append-FormattedText $tr "01. INGESTION & INTERCEPTION`r`r" "Segoe UI" 14.5 $COLOR_CYAN $true | Out-Null
    Append-FormattedText $tr "Non-Invasive Gateway Interceptor`r`r" "Segoe UI" 17 $COLOR_WHITE $true | Out-Null
    $body1 = "- Transparent HTTP Wrapper: Utilizes CachedBodyHttpServletRequest to safely inspect inbound payload bodies without prematurely consuming the downstream stream.`r`r- Full-Spectrum Inspection: Parses and correlates HTTP headers, URI query params, route variables, and JSON payloads simultaneously.`r`r- Zero Disruption: Operates seamlessly as an upstream reverse proxy or embedded servlet filter with under 5ms latency overhead."
    Append-FormattedText $tr $body1 "Segoe UI" 13 $COLOR_TEXT_BODY $false | Out-Null
    ...
    $tr = $tf.TextRange
    Append-FormattedText $tr "02. THREAT DETECTION & SCORING`r`r" "Segoe UI" 14.5 $COLOR_GREEN $true | Out-Null
    Append-FormattedText $tr "Multi-Vector Heuristic Engine`r`r" "Segoe UI" 17 $COLOR_WHITE $true | Out-Null
    $body2 = "- OWASP Top 10 Coverage: Modular detectors scan for SQL Injection, XSS payloads, Credential Abuse, and Sensitive Data Exposure.`r`r- IDOR / BOLA Discovery: Sequential path anomaly detection flags automated resource harvesting and parameter enumeration.`r`r- Dynamic 0-100 Scoring: Weights vector confidence, historical IP reputation, and payload lethality to produce an instant threat score."
    Append-FormattedText $tr $body2 "Segoe UI" 13 $COLOR_TEXT_BODY $false | Out-Null
    ...
    $tr = $tf.TextRange
    Append-FormattedText $tr "03. MITIGATION & OBSERVABILITY`r`r" "Segoe UI" 14.5 $COLOR_CYAN $true | Out-Null
    Append-FormattedText $tr "Zero-Trust Policy Enforcement`r`r" "Segoe UI" 17 $COLOR_WHITE $true | Out-Null
    $body3 = "- Autonomous Response: Instant verdict determination - ALLOWED, RATE_LIMITED (HTTP 429), or BLOCKED (HTTP 403 Forbidden).`r`r- Perimeter IP Isolation: Dynamically quarantines aggressive IPs at the gateway perimeter before microservices are impacted.`r`r- Real-Time SOC Telemetry: Next.js operations console with live threat feeds, forensic payload audits, and interactive attack simulator."
    Append-FormattedText $tr $body3 "Segoe UI" 13 $COLOR_TEXT_BODY $false | Out-Null
    ...
    # Q1: Frontend
    $q1 = $slide4.Shapes.AddShape(5, $gStartX, $gStartY, $gridW, $gridH)
    $q1.Fill.Solid(); $q1.Fill.ForeColor.RGB = $COLOR_CARD_BG
    $q1.Line.ForeColor.RGB = $COLOR_CYAN; $q1.Line.Weight = 1.5
    $tf = $q1.TextFrame; $tf.MarginLeft = 25; $tf.MarginRight = 25; $tf.MarginTop = 20
    $tr = $tf.TextRange
    Append-FormattedText $tr "FRONTEND : SECURITY OPERATIONS CENTER (SOC)`r" "Segoe UI" 15.5 $COLOR_CYAN $true | Out-Null
    $t1 = "- Stack: Next.js 14 (App Router), React 18, TypeScript, Tailwind CSS, Lucide Icons.`r- Live Telemetry: Sub-second telemetry aggregation displaying active threats, request verdicts, risk trends, and endpoints.`r- Live Production Deployment: Hosted on Vercel Edge Network at https://apisentinel-psi.vercel.app`r- Interactive Console: Full audit exploration, policy controls, and offensive attack simulator."
    Append-FormattedText $tr $t1 "Segoe UI" 12.5 $COLOR_TEXT_BODY $false | Out-Null

    # Q2: Backend
    $q2 = $slide4.Shapes.AddShape(5, $gStartX + $gridW + $gridGapX, $gStartY, $gridW, $gridH)
    $q2.Fill.Solid(); $q2.Fill.ForeColor.RGB = $COLOR_CARD_BG
    $q2.Line.ForeColor.RGB = $COLOR_GREEN; $q2.Line.Weight = 1.5
    $tf = $q2.TextFrame; $tf.MarginLeft = 25; $tf.MarginRight = 25; $tf.MarginTop = 20
    $tr = $tf.TextRange
    Append-FormattedText $tr "BACKEND : ZERO-TRUST API SECURITY GATEWAY`r" "Segoe UI" 15.5 $COLOR_GREEN $true | Out-Null
    $t2 = "- Stack: Java 17, Spring Boot 3.2, Spring Security 6 (Stateless Zero-Trust Filter Chain).`r- Resilient Architecture: HikariCP connection pooling, graceful shutdowns, and Actuator health probes.`r- Cloud Deployment: Containerized multi-stage Docker build deployed on Render at https://api-backend-wc8m.onrender.com`r- Performance: Non-blocking in-memory evaluation with under 5ms processing latency per request."
    Append-FormattedText $tr $t2 "Segoe UI" 12.5 $COLOR_TEXT_BODY $false | Out-Null

    # Q3: Detection
    $q3 = $slide4.Shapes.AddShape(5, $gStartX, $gStartY + $gridH + $gridGapY, $gridW, $gridH)
    $q3.Fill.Solid(); $q3.Fill.ForeColor.RGB = $COLOR_CARD_BG
    $q3.Line.ForeColor.RGB = $COLOR_GREEN; $q3.Line.Weight = 1.5
    $tf = $q3.TextFrame; $tf.MarginLeft = 25; $tf.MarginRight = 25; $tf.MarginTop = 20
    $tr = $tf.TextRange
    Append-FormattedText $tr "CORE DETECTION & SCORING PIPELINE`r" "Segoe UI" 15.5 $COLOR_GREEN $true | Out-Null
    $t3 = "- Interception Filter: Intercepts raw byte streams and constructs thread-safe ApiRequestContext.`r- Modular Detectors: InjectionDetector (SQLi/XSS), EnumerationDetector (BOLA), AuthenticationAbuseDetector, RateAbuseDetector, SensitiveDataExposureDetector.`r- Scoring Engine: ThreatScoringEngine evaluates weighted vector risk into deterministic score (0-100).`r- Enforcement Engine: DefaultPolicyEngine matches active rules and returns instant verdict."
    Append-FormattedText $tr $t3 "Segoe UI" 12.5 $COLOR_TEXT_BODY $false | Out-Null

    # Q4: Persistence
    $q4 = $slide4.Shapes.AddShape(5, $gStartX + $gridW + $gridGapX, $gStartY + $gridH + $gridGapY, $gridW, $gridH)
    $q4.Fill.Solid(); $q4.Fill.ForeColor.RGB = $COLOR_CARD_BG
    $q4.Line.ForeColor.RGB = $COLOR_CYAN; $q4.Line.Weight = 1.5
    $tf = $q4.TextFrame; $tf.MarginLeft = 25; $tf.MarginRight = 25; $tf.MarginTop = 20
    $tr = $tf.TextRange
    Append-FormattedText $tr "PERSISTENCE & MONOREPO CLOUD ARCHITECTURE`r" "Segoe UI" 15.5 $COLOR_CYAN $true | Out-Null
    $t4 = "- Cloud Database: Managed PostgreSQL database on Render storing security incidents, request audit logs, endpoints, and firewall policies.`r- Single GitHub Monorepo: Houses /backend, /frontend, /database, and orchestration in one repo.`r- Local Development: docker-compose.yml runs complete local MySQL/Spring/Next.js environment.`r- Cross-Origin Security: Fine-grained CORS controls securing browser clients and API gateways."
    Append-FormattedText $tr $t4 "Segoe UI" 12.5 $COLOR_TEXT_BODY $false | Out-Null
    ...
    $steps = @(
        @{ num = "STEP 1"; title = "Inbound Request Ingestion"; desc = "Client or attacker issues HTTP API call (GET, POST, PUT, DELETE) targeting protected microservice endpoints."; color = $COLOR_CYAN },
        @{ num = "STEP 2"; title = "Deep Payload Caching"; desc = "TrafficInterceptionFilter wraps stream using CachedBodyHttpServletRequest and extracts headers, URI parameters, and body."; color = $COLOR_GREEN },
        @{ num = "STEP 3"; title = "Multi-Vector Heuristics"; desc = "Engine executes parallel modular detectors scanning for SQL injection tokens, XSS scripts, token leakage, and BOLA enumeration patterns."; color = $COLOR_CYAN },
        @{ num = "STEP 4"; title = "Dynamic Threat Scoring"; desc = "ThreatScoringEngine aggregates detection signals and evaluates a composite 0-100 severity score with recommended action."; color = $COLOR_GREEN },
        @{ num = "STEP 5"; title = "Automated Policy Action"; desc = "PolicyEngine decides verdict: ALLOWED (forwarded to downstream handler), RATE_LIMITED (HTTP 429), or BLOCKED (HTTP 403 Forbidden)."; color = $COLOR_CYAN },
        @{ num = "STEP 6"; title = "Forensic Log & SOC Update"; desc = "Request audit details and forensic evidence are persisted to PostgreSQL and pushed live to the SOC Dashboard in real time."; color = $COLOR_GREEN }
    )
    ...
    for ($idx = 0; $idx -lt $steps.Count; $idx++) {
        ...
        $col = $idx % 3
        $x = $sStartX + $col * ($stepW + $stepGapX)
        $y = $sStartY + $row * ($stepH + $stepGapY)

        $sc = $slide5.Shapes.AddShape(5, $x, $y, $stepW, $stepH)
        $sc.Fill.Solid(); $sc.Fill.ForeColor.RGB = $COLOR_CARD_BG
        $sc.Line.ForeColor.RGB = $st.color; $sc.Line.Weight = 1.5
        $tf = $sc.TextFrame; $tf.MarginLeft = 25; $tf.MarginRight = 25; $tf.MarginTop = 25
        $tr = $tf.TextRange
        Append-FormattedText $tr "$($st.num) : $($st.title)`r`r" "Segoe UI" 14.5 $st.color $true | Out-Null
        Append-FormattedText $tr $st.desc "Segoe UI" 13 $COLOR_TEXT_BODY $false | Out-Null
    }
    ...
    if (Test-Path $img1Path) {
        ...
        Append-FormattedText $tr "Security Operations Center (SOC) Dashboard`r" "Segoe UI" 15.5 $COLOR_CYAN $true | Out-Null
    ...
    if (Test-Path $img2Path) {
        ...
        Append-FormattedText $tr "Interactive Attack Simulator & Forensic Inspector`r" "Segoe UI" 15.5 $COLOR_GREEN $true | Out-Null
    ...
    $tr = $tf.TextRange
    Append-FormattedText $tr "LIVE DEPLOYED DEMO LINKS (CLICKABLE IN SUBMISSION) :`r" "Segoe UI" 13.5 $COLOR_GREEN $true | Out-Null
    ...
    $tr = $tf.TextRange
    Append-FormattedText $tr "TECHNICAL FEASIBILITY & CLOUD-NATIVE SCALABILITY`r" "Segoe UI" 16 $COLOR_CYAN $true | Out-Null
    ...
    $tr = $tf.TextRange
    Append-FormattedText $tr "COMMERCIAL VIABILITY & MARKET ADOPTION`r" "Segoe UI" 16 $COLOR_GREEN $true | Out-Null
    ...
    $tr = $tf.TextRange
    Append-FormattedText $tr "MEASURABLE IMPACT & SOC EMPOWERMENT`r" "Segoe UI" 16 $COLOR_CYAN $true | Out-Null
    ...
    Append-FormattedText $tr "LIVE DEMO PLATFORM`r" "Segoe UI" 21 $COLOR_GREEN $true | Out-Null
    ...
    Append-FormattedText $tr "https://apisentinel-psi.vercel.app" "Segoe UI" 15.5 $COLOR_WHITE $true | Out-Null
    try {
    ...
    $tr.ParagraphFormat.Alignment = 1
    Append-FormattedText $tr "PLATFORM CAPABILITIES:`r`r" "Segoe UI" 14.5 $COLOR_GREEN $true | Out-Null
    ...
    $tr.ParagraphFormat.Alignment = 1
    Append-FormattedText $tr "GITHUB REPOSITORY`r" "Segoe UI" 21 $COLOR_CYAN $true | Out-Null
    ...
    $tr.ParagraphFormat.Alignment = 2
    Append-FormattedText $tr "https://github.com/Yokeshwaran03-2005/API-SENTINEL" "Segoe UI" 14 $COLOR_WHITE $true | Out-Null
    try {
    ...
    $tr.ParagraphFormat.Alignment = 1
    Append-FormattedText $tr "REPOSITORY CONTENTS:`r`r" "Segoe UI" 14.5 $COLOR_CYAN $true | Out-Null
    ...
    Write-Host "SUCCESS: Presentation and PDF generated cleanly with universal Segoe UI fonts!"
